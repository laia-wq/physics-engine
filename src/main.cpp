...
constexpr std::size_t MAX_PHYSICS_STEPS_PER_FRAME = 2;
...
using app::CircleView;
using app::ParticleMaterial;
using app::materialBreakingScale;
using app::materialColor;
using app::materialForceResponse;
using app::materialName;
...
int main()
{
    ...
    const auto instantiateConnectedPreset = [&](const presets::ConnectedPreset& preset)
    {
        ...
        for (const auto& definition : preset.bodies)
        {
            ...
            bodies.back().material =
                static_cast<ParticleMaterial>(definition.material);
            bodies.back().groupId = definition.groupId;
            bodies.back().showFixedOutline = definition.showFixedOutline;
        }
    ...
    const auto loadParticleApple = [&]()
    {
        instantiateConnectedPreset(
            presets::buildParticleApple(windowWidth, windowHeight)
        );
    ...
    {
        presets::buildRollingTerrain(
            bodies, springs, terrainMeshDiagonals
        );
    ...
    while (window.isOpen())
    {
        ...
        if (ImGui::CollapsingHeader("Performance"))
        {
            ...
            if (physicsBacklogDropped)
            {
                ImGui::TextColored(
                    ImVec4(1.f, 0.75f, 0.25f, 1.f),
                    "Heavy scene: keeping controls responsive"
                );
            }
        ...
        std::size_t physicsStepsThisFrame = 0;
        physicsBacklogDropped = false;
        while (!paused && accumulator >= FIXED_TIME_STEP &&
               physicsStepsThisFrame < MAX_PHYSICS_STEPS_PER_FRAME)
        {
            simulateStep();
        ...
        if (!paused && accumulator >= FIXED_TIME_STEP)
        {
            // A complex scene can cost more than the 8.33 ms available for a
            // 120 Hz step. Discard overdue whole steps instead of repeatedly
            // trying to catch up and starving input/rendering indefinitely.
            accumulator = std::fmod(accumulator, FIXED_TIME_STEP);
